                 ...
                 uint32 clusterSize)
{
    ...
    for (i = 0; i < RTL865X_SWNIC_RXRING_MAX_PKTDESC; i++) {
		if (rxPkthdrRingCnt[i] == 0)
			continue;
		flush_dcache_range(rxPkthdrRing[i], (rxPkthdrRing[i] + rxPkthdrRingCnt[i] * sizeof(uint32)+32));
		rxPkthdrRing[i] = (0xa0000000 | (uint32)(rxPkthdrRing[i]));
    ...
    for (i = 0; i < RTL865X_SWNIC_TXRING_MAX_PKTDESC; i++) {
		...
		flush_dcache_range(txPkthdrRing[i], (txPkthdrRing[i] + txPkthdrRingCnt[i] * sizeof(uint32)+32));	
		txPkthdrRing[i] = (0xa0000000 | (uint32)(txPkthdrRing[i]));
		txPkthdrRing[i] = PTR_ALIGN(txPkthdrRing[i], 32);
    }

	flush_dcache_range(rxMbufRing, (rxMbufRing+ userNeedRxMbufRingCnt * sizeof(uint32)+32));
	rxMbufRing = (0xa0000000 | (uint32)(rxMbufRing));
	rxMbufRing = PTR_ALIGN(rxMbufRing, 32);
	flush_dcache_range(pPkthdrList, (pPkthdrList+ (totalRxPkthdrRingCnt + totalTxPkthdrRingCnt) * sizeof(struct pktHdr)+32));	
	pPkthdrList = (0xa0000000 | (uint32)(pPkthdrList));
	pPkthdrList = PTR_ALIGN(pPkthdrList, 32);
	flush_dcache_range(pMbufList, (pMbufList+  (rxMbufRingCnt + totalTxPkthdrRingCnt) * sizeof(struct mBuf)+32));
	pMbufList = (0xa0000000 | (uint32)(pMbufList));
	...
	pClusterList = (0xa0000000 | (uint32)(pClusterList));
